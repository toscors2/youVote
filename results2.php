...
/*prepares voting query*/
$votes2 = $conn->prepare("select contestant from voiceVotes group by ticketNum");
$votes2->execute();
$votes2->store_result();
$votes2->bind_result($contestant);
...
echo "<div style='position:relative; width:100%; margin-left:50%; transform: translate(-50%);'><img id='refresh' src='img/contestants/refresh.png' style='width:250px; height:75px; margin:0 30px;'><img style='width:250px; height:75px; margin:0 30px;' id='reVote' src='img/contestants/voteAgain.png'>". $uniqueCount ."</div>";
foreach ($vote as $print) {
    ...
    echo "<div class='resultsGraph' style='width:" .$percent ."%;'>". $percent ."%:  Votes: " .$print['count'] . " of ".$totalCount."</div>";
    echo "</div></div>";
}
